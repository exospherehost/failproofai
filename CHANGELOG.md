...
### Features
- Add OpenCode (sst/opencode) integration (beta). New `--cli opencode` installs into `~/.config/opencode/` (user) or `<cwd>/.opencode/` (project), writing both a generated plugin shim at `plugins/failproofai.mjs` and a registration entry in `opencode.json`'s `plugin: []` array. OpenCode's plugin model is in-process JS/TS (no external-command hooks like the other four CLIs) — the shim subprocess-calls the failproofai binary and translates the existing Claude-shape JSON response back into plugin semantics (`throw new Error()` for deny, `client.session.prompt(...)` for instruct, no-op for allow). Subscribes to `tool.execute.before` / `tool.execute.after` / `permission.ask` (first-class hooks) plus the generic `event` handler for `session.created` / `session.deleted` / `session.idle` / `message.updated` (filtered to `role:user`). Path-protection extended to cover `.opencode/`, `~/.config/opencode/`, and `~/.local/share/opencode/`. Frontend: amber CLI badge in `lib/cli-registry.ts`; new DB-shell adapters (`lib/opencode-projects.ts`, `lib/opencode-sessions.ts`) read opencode's SQLite store via `opencode db --format json` and `opencode export <id>`; `/projects` and the session viewer merge OpenCode entries alongside the other four CLIs. Also ships this repo's own `.opencode/opencode.json` + `.opencode/plugins/failproofai.mjs` so contributors using OpenCode get hooks active automatically (#266)
