...
### Fixes
- `failproofai policies --uninstall` (and the `-u` alias) interactive CLI selector now reads `Failproof AI — Remove Hooks` and `Choose where to remove from:` instead of mistakenly saying "Install Hooks" / "Choose where to install:". `resolveTargetClis` accepts an `action: "install" | "uninstall"` parameter that flows through `bin/failproofai.mjs` and into the prompt heading + non-interactive log line ("removing hooks from <CLI>" vs "installing hooks for <CLI>"). Reported during the GitHub Copilot rollout — this was a pre-existing bug visible whenever multiple agent CLIs are detected during uninstall.
