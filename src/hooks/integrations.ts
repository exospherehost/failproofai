...
export const TOOL_NAME_CANONICAL_MAP: Record<string, string> = {
  // File write variants
  "writefile":             "Write",
  "write_file":            "Write",
  "save_file":             "Write",
  ...
  // Shell variants → canonical "Bash"
  "shell":                 "Bash",
  "terminal":              "Bash",
  "console":               "Bash",
  "sh":                    "Bash",
  "bash":                  "Bash",
  "bash_login_shell":      "Bash",
  "run_terminal_command":  "Bash",
  "run_shell_command":     "Bash",
  "execute_command":       "Bash",
};

export function canonicalizeToolName(name: string | undefined): string | undefined {
  if (!name) return name;
  return TOOL_NAME_CANONICAL_MAP[name.toLowerCase()] ?? name;
}
...
`;
  ...
  hooksInstalledInSettings(scope: string, cwd?: string): boolean {
    const path = this.getSettingsPath(scope, cwd);
    if (!existsSync(path)) return false;
    try {
      const content = readFileSync(path, "utf8");
      return content.includes(this.hookMarker);
    } catch {
      return false;
    }
  },
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_result", (event, ctx) => {
    try {
      const toolName = event.toolName || event.name;
      const toolInput = event.input || event.args || event.arguments;
      let toolOutput = event.result?.content || event.output || event.result;
  ...
  pi.on("message", (event, ctx) => {
    try {
      if (event.role === "assistant") {
        const content = event.content || event.text || (typeof event.content === "object" ? JSON.stringify(event.content) : "");
        if (content) {
          callcli("AssistantResponse", { 
        ...
        if (event.stopReason === "stop" || event.stopReason === "end_turn") {
          callcli("stop", { session_id: event?.sessionId || event?.session_id || event?.session?.id }, ctx);
        }
      }
    } catch {}
  });
