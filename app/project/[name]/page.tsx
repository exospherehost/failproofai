...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  const [codex, copilot, cursor, pi] = await Promise.all([
  ...
  // round-trip correctly. First non-null wins (Codex → Copilot → Cursor → Pi).
  const canonicalRoot = codex.cwd ?? copilot.cwd ?? cursor.cwd ?? pi.cwd ?? decodedName;
  ...
  const newestExternal = [codexSessions[0], copilotSessions[0], cursorSessions[0], piSessions[0]]
