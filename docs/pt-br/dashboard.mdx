...
O visualizador de sessão responde à pergunta central sobre agentes autônomos: o que o agente fez e ele permaneceu no caminho certo? Ele exibe uma linha do tempo de tudo o que aconteceu em uma sessão:
...
<Tabs>
  ...
  - Histórico completo e paginado de todos os eventos de hook disparados em todas as sessões
  - Pesquise por nome de política, ID de sessão, nome de ferramenta ou decisão
  - Cada linha exibe: timestamp, nome da política, decisão, nome da ferramenta, ID de sessão e o motivo para decisões deny/instruct
  </Tab>
</Tabs>
...
O dashboard possui um botão de atualização automática na navegação superior. Quando habilitado, a página atual é atualizada periodicamente para exibir novas sessões e atividades de políticas à medida que aparecem. Essencial para monitorar sessões de agentes autônomos de longa duração.
...
Ao executar o dashboard em **modo de desenvolvimento** (`npm run dev`) e acessá-lo a partir de um hostname diferente de `localhost` — por exemplo, um domínio personalizado, um IP remoto ou uma URL tunelada — você poderá ver um aviso como:
