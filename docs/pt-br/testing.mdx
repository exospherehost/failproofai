---
title: Testes
description: "Testes unitários, testes E2E e helpers de teste"
icon: flask-vial
---
...
__tests__/
  hooks/
    ...
    hooks-config.test.ts          # Carregamento de config e merge de escopos
...
Os testes E2E executam o binário diretamente a partir do código-fonte do repositório. Antes da primeira execução, compile o bundle CJS que os arquivos de hook customizados usam ao importar de `'failproofai'`:
...
__tests__/e2e/
  helpers/
    ...
    fixture-env.ts      # Diretórios temporários isolados por teste com arquivos de config
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Merge de config entre escopos project/local/global
...
// env.home   - home dir isolado; sem vazamento do ~/.failproofai real
...
`createFixtureEnv()` registra o cleanup via `afterEach` automaticamente.
...
- `testTimeout: 20_000` - 20s por teste (inicialização do binário + avaliação de hook)

O pool `forks` é importante: workers baseados em threads compartilham o `globalThis`, o que pode interferir com testes que iniciam subprocessos. Forks baseados em processos evitam esse problema.

---

## CI

A execução completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) precisa passar antes do merge. O conjunto de testes E2E é executado como um job de CI separado em paralelo.
