...
__tests__/
  hooks/
    ...
    custom-hooks-loader.test.ts   # Loader ESM, importações transitivas, tratamento de erros
...
Os testes E2E invocam o binário real do `failproofai` como subprocesso, enviam um payload JSON via stdin e verificam a saída no stdout e o código de saída. Isso testa o caminho completo de integração utilizado pelo Claude Code.
...
Recompile o `dist/` sempre que você alterar a API pública de hooks (`src/hooks/custom-hooks-registry.ts`, `src/hooks/policy-helpers.ts` ou `src/hooks/policy-types.ts`).
...
// env.home   - home dir isolado; evita vazamentos do ~/.failproofai real
...
- `environment: "node"` - sem globals de navegador necessários
...
A execução completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) deve passar antes de qualquer merge. A suíte E2E é executada como um job separado de CI em paralelo.
