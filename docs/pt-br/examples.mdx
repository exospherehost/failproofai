...
Failproof AI se integra ao Claude Code por meio do seu [sistema de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Quando você executa `failproofai policies --install`, ele registra comandos de hook no `settings.json` do Claude Code que são disparados a cada chamada de ferramenta.
...
<Steps>
  ...
  <Step title="Verifique se os hooks estão registrados">
    ```bash
...
Essas políticas disparam no `PostToolUse` — após a execução de uma ferramenta, elas limpam a saída antes que o agente a veja.
...
Faça commit de uma configuração no nível do projeto para que todos os desenvolvedores do time utilizem as mesmas políticas.
...
Todo membro do time que tiver o failproofai instalado receberá essas regras automaticamente.
...
<Steps>
  ...
  <Step title="Continue melhorando">
    À medida que o time encontrar novos pontos de falha, adicione políticas e faça push. Todos recebem a atualização no próximo `git pull`. Essas políticas se tornam um padrão de qualidade vivo que cresce junto com o time.
  </Step>
</Steps>
