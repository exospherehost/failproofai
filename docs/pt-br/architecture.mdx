...
1. **Handler de hooks** - Um subprocesso CLI rápido que o Claude Code invoca a cada chamada de ferramenta do agente. Avalia políticas e retorna uma decisão.
2. **Monitor de Agente (Dashboard)** - Uma aplicação web Next.js para monitorar sessões do agente e gerenciar políticas.
...
Quando você executa `failproofai policies --install`, ele grava entradas como esta em `~/.claude/settings.json`:
...
O handler impõe um limite de 1 MB para o stdin. Payloads que excedam esse limite são descartados e todas as políticas permitem implicitamente.
...
**Evento Stop com instrução:**
- Código de saída: `2`
- Motivo gravado em stderr (não em stdout)
...
`allow(message)` permite que uma política envie contexto informativo de volta ao Claude mesmo quando a operação é permitida. O handler de hooks grava o seguinte JSON no **stdout** (não em um arquivo de configuração — esta é a resposta do handler ao Claude Code, assim como as respostas de deny e instruct acima):
...
- `policyParams` - por chave de política, o primeiro arquivo que a definir prevalece inteiramente
- `customPoliciesPath` - o primeiro arquivo que o definir prevalece
- `llm` - o primeiro arquivo que o definir prevalece

O dashboard web usa `readHooksConfig()` (apenas global) para leitura e escrita, pois não é invocado com um cwd de projeto.
...
1. Busca o esquema `params` da política (se ela tiver um).
...
- Se algum `instruct` foi coletado, emite uma única resposta de instruct com todas as mensagens unidas.
...
Políticas que aceitam `params` declaram um `PolicyParamsSchema` com tipos e valores padrão para cada parâmetro. O avaliador de políticas injeta os valores resolvidos em `ctx.params` antes de chamar `fn`. As funções de política leem `ctx.params` sem verificações de nulo, pois os padrões são sempre aplicados primeiro.

A correspondência de padrões dentro das políticas usa tokens de comando analisados (argv), não correspondência de strings brutas. Isso previne bypass via injeção de operadores shell (por exemplo, um padrão para `sudo systemctl status *` não pode ser contornado acrescentando `; rm -rf /` ao comando).
...
Em caso de qualquer erro (arquivo não encontrado, erro de sintaxe, falha de importação), o erro é registrado em `~/.failproofai/hook.log` e o carregador retorna um array vazio. As políticas embutidas não são afetadas.
...
Após cada evento de hook, o handler acrescenta uma linha JSONL a `~/.failproofai/hook-activity.jsonl`:
...
app/
  ...
  policies/page.tsx           ← Client component: gerenciamento de políticas + log de atividades
  actions/
  ...
  api/
    download/[project]/[session]/route.ts   ← Exportação de sessão CLI (JSONL ou JSON)
```
...
- React Server Components para páginas de leitura - carregamento inicial mais rápido, sem bundle cliente para busca de dados.
- Client components apenas onde há necessidade de interatividade (alternância de políticas, busca de atividade, visualizador de logs).
