...
Cada política ocupa um slot `<namespace>/<nome>`. As políticas integradas pertencem ao namespace **`exospherehost/`** — por exemplo, `exospherehost/sanitize-jwt`. O namespace evita colisões quando você também carrega políticas personalizadas ou de terceiros com nomes curtos semelhantes.
...
Se um nome não contiver `/`, failproofai o trata como pertencente ao namespace padrão `exospherehost`. Nomes que já contêm `/` (ex.: `myorg/foo`, `custom/my-hook`) são mantidos como estão.
...
Bloqueia invocações que incluem a palavra-chave `sudo`. A correspondência de padrão é feita em tokens de comando analisados, não na string bruta, para evitar bypass via injeção de operador shell.
...
Impede que agentes de codificação executem CLIs de infraestrutura ou acionem pipelines de CI/CD. Todas as políticas nesta categoria são **opt-in** (`defaultEnabled: false`) — agentes que legitimamente precisam chamar `kubectl`, `terraform`, etc. não serão perturbados a menos que você habilite a política. Quando habilitada, toda invocação do CLI correspondente é negada, a menos que o comando corresponda a uma entrada em `allowPatterns`.

A gramática de padrões é a mesma de [`block-sudo`](#block-sudo): os tokens são comparados com argv analisado, `*` é um curinga para um token, e qualquer comando contendo um operador shell independente (`&&`, `||`, `|`, `;`) ou um token com metacaracteres shell incorporados é rejeitado antes da correspondência da lista de permissões para evitar bypasses de injeção.
...
**Evento:** PreToolUse (Bash)  
**Padrão:** Instrui Claude a revisar o que está adicionando ao stage ao executar `git add -A` ou `git add .`. Não bloqueia o comando.
...
Detecta operações SQL destrutivas antes que sejam executadas contra o banco de dados.
...
<Note>
O handler do hook impõe um limite de 1 MB de stdin nos payloads. Para testar esta política com conteúdo pequeno, defina `thresholdKb` para um valor bem abaixo de 1024.
</Note>
...
A aplicação de Stop funciona de forma ligeiramente diferente entre os sete CLIs suportados porque cada um expõe um contrato de hook diferente para "agente finalizado". O **resultado** é o mesmo — o agente não consegue parar enquanto um gate de fluxo de trabalho está falhando — mas a **mecânica** difere. A tabela abaixo resume; apenas o Pi tem uma peculiaridade visível ao usuário que vale entender antes de habilitar uma política `require-*-before-stop`.

| CLI | Quando o gate é acionado | O que você vê |
|---|---|---|
| Claude Code | Mesmo loop do agente, imediatamente | Claude continua trabalhando — corrige o problema e então tenta finalizar novamente. Sem interrupção visível para você. |
...
| Cursor Agent | Mesmo loop do agente, imediatamente | Igual ao Claude (usa o canal `{followup_message}` do Cursor — limitado ao `loop_limit`, padrão de 5 tentativas). |
...
| **Pi (pi-coding-agent)** | **Próximo turno do usuário** | **Pi para visivelmente** quando o gate é acionado — seu loop de agente sai e você retorna ao prompt. O gate então é acionado na próxima vez que você envia um prompt: failproofai adiciona uma diretiva `MANDATORY ACTION REQUIRED` ao início do prompt de sistema daquele turno, instruindo o LLM a concluir a etapa do fluxo de trabalho (commit, push, etc.) antes de fazer o que você pediu. |

<Note>
**Limitação do Pi.** O `AgentEndEvent` do Pi (equivalente upstream ao hook `Stop` do Claude) não tem tipo Result — quando ele é acionado, o loop do agente do Pi já saiu. O Pi não pode ser forçado a tentar novamente o mesmo loop da forma que Claude / Copilot / Cursor / Gemini / OpenCode podem. O failproofai desloca o gate para o evento `before_agent_start` do Pi (que é acionado após o próximo prompt do usuário) para que a verificação do fluxo de trabalho ainda seja imposta, apenas no próximo turno em vez do atual.

**O que isso significa na prática:**

- Após o Pi parar, o motivo de deny é capturado em memória com chave pelo id de sessão do Pi. O próximo prompt que você enviar no mesmo processo Pi o consome: o LLM vê a diretiva `MANDATORY ACTION REQUIRED` no topo do seu prompt de sistema, faz commit (ou push / abre o PR / aguarda CI), e só então continua com seu pedido. O motivo de deny capturado é de uso único — uma vez consumido, o gate é liberado.
- O gate é limitado pelo tempo de vida do processo do Pi. Se você usar `Ctrl+C` no Pi ou sair entre turnos, a entrada em memória é descartada junto com o processo e o gate é perdido. Claude, Copilot, Cursor, Gemini e OpenCode têm o mesmo limite (encerrar o agente faz o gate ser perdido) — o Pi apenas torna isso mais visível porque o agente sai visivelmente antes que o gate seja acionado.
- Um deny pendente também é limpo em `session_shutdown` por qualquer motivo (`new` / `resume` / `fork` / `quit`), portanto um gate obsoleto de uma sessão anterior não pode vazar para uma nova sessão iniciada no mesmo processo Pi.

Se você precisar de retry no mesmo loop no estilo Claude, execute suas políticas `Stop` em qualquer um dos outros seis CLIs suportados. Estamos acompanhando o Pi upstream para um futuro tipo Result em `AgentEndEvent` que nos permitiria fechar essa lacuna.
</Note>
...
**Evento:** Stop  
**Padrão:** Nega a parada quando há commits não enviados ou quando o branch atual não tem branch de rastreamento remoto. Sugere `git push -u` para criar um branch de rastreamento se necessário. Fail-open se nenhum remote estiver configurado.
...
**Evento:** Stop  
**Padrão:** Nega a parada quando não existe pull request para o branch atual, ou quando o PR existente está fechado sem merge. Instrui Claude a criar um PR com `gh pr create`. Quando o PR está **mergeado**, a política permite (o trabalho foi entregue) e a mensagem sugere sair do branch (`git checkout main && git pull`).
...
pull requests. Se `gh` não estiver instalado ou não estiver autenticado, a política falha de forma aberta e reporta o motivo ao Claude.
</Note>
...
**Evento:** Stop  
**Padrão:** Nega a parada quando o branch atual não pode ser mergeado de forma limpa no branch base. A política primeiro confirma que existe um PR `OPEN` no GitHub para o branch — sem um, não há destino de merge a impor, portanto toda a política curto-circuita para allow. Uma vez que um PR `OPEN` é confirmado, duas sondagens independentes são executadas:
...
2. **GitHub** — reutiliza o resultado de `gh pr view --json mergeable,state` já obtido na pré-verificação. Detecta conflitos que um `origin/<baseBranch>` local desatualizado perderia (ex.: alguém entregou um PR conflitante em `main` desde o último fetch). Um resultado `CONFLICTING` nega. Um resultado `UNKNOWN` também nega e instrui Claude a aguardar ~10 segundos e verificar novamente antes de tentar parar — isso evita falsos negativos enquanto o GitHub recalcula.

Ignora completamente (permite) quando: `gh` não está instalado, não existe PR para o branch, o estado do PR não é `OPEN` (ex.: `MERGED`, `CLOSED`), ou `gh pr view` retorna saída não parseável. Também fail-open quando `origin/<baseBranch>` está ausente localmente ou quando não há commits à frente do base — esses fall-throughs da Camada 1 ainda consultam a mergeabilidade do PR em cache antes de permitir.
...
curto-circuita para allow. Execute `gh auth login` com um token de acesso pessoal que tenha
...
execuções de workflow do Actions e à API de Checks. Se `gh` não estiver instalado ou não estiver autenticado, a política falha de forma aberta e reporta o motivo ao Claude.
</Note>
