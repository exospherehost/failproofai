---
title: 패키지 별칭
description: "등록된 오타 방지 별칭과 그 동작 방식"
icon: copy
---
...
타이포스쿼팅(typosquatting)은 공급망 공격의 일반적인 수법으로, 악의적인 행위자가 인기 패키지 이름과 한 글자 차이나는 패키지명을 등록합니다. 설치 명령어를 잘못 입력한 사용자는 전체 시스템 접근 권한을 가진 공격자 제어 코드를 실행하게 됩니다 — 이것이 바로 Failproof AI가 방어하도록 설계된 위협의 전형적인 예입니다.

이러한 공격 표면을 제거하기 위해, **저희는 npm에서 `failproofai`의 일반적인 오타 및 표기 변형을 모두 선제적으로 소유하고 있습니다**. 이 이름들 중 어느 것도 제3자가 등록할 수 없습니다. 각 이름은 실제 `failproofai` 패키지를 설치하고 위임하는 얇은 프록시입니다.
...
1. `failproofai`를 의존성으로 등록하여 설치 시 실제 패키지(`postinstall` 훅 설정 포함)가 실행되도록 합니다
