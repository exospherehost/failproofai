...
| Stop instruct | `2` | 빈 stdout; 이유는 stderr에 출력 |
| Allow | `0` | 빈 문자열 |
...
`forks` 풀은 중요합니다: 스레드 기반 워커는 `globalThis`를 공유하므로 서브프로세스를 생성하는 테스트에 영향을 줄 수 있습니다. 프로세스 기반의 포크는 이 문제를 방지합니다.
...
머지 전 전체 체크리스트는 [Contributing](../CONTRIBUTING.md)을 참조하세요.