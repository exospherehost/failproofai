---
title: 예제
description: "Claude Code와 Agents SDK를 위한 훅 설정 방법"
icon: book-open
---

일반적인 시나리오에 바로 사용할 수 있는 예제 모음입니다. 각 예제에는 설치 방법과 예상 동작이 포함되어 있습니다.

---

## Claude Code에 훅 설정하기

Failproof AI는 Claude Code의 [훅 시스템](https://docs.anthropic.com/en/docs/claude-code/hooks)을 통해 통합됩니다. `failproofai policies --install`을 실행하면 Claude Code의 `settings.json`에 훅 명령이 등록되어 모든 도구 호출 시 실행됩니다.
...
<Steps>
  ...
  <Step title="에이전트에 훅 설정">
    에이전트 프로세스를 생성할 때 훅 명령을 전달하세요. Claude Code와 동일하게 stdin/stdout JSON을 통해 훅이 실행됩니다:
...
가장 일반적인 설정 — 에이전트가 되돌릴 수 없는 손상을 입히는 것을 방지합니다.
...
- `block-curl-pipe-sh` — 원격 스크립트를 쉘로 파이프하는 것 차단

---

## 시크릿 유출 방지

에이전트가 도구 출력에서 자격 증명을 보거나 유출하는 것을 차단합니다.
...
팀의 모든 개발자가 동일한 정책을 적용받도록 프로젝트 수준 설정을 커밋합니다.
...
failproofai가 설치된 모든 팀원은 이 규칙을 자동으로 적용받게 됩니다.

---

## 컨벤션 정책으로 조직 전체 품질 기준 구축

가장 효과적인 설정: 프로젝트에 맞게 조정된 정책을 담은 `.failproofai/policies/`를 저장소에 커밋합니다. 모든 팀원이 별도의 설치 명령이나 설정 변경 없이 자동으로 정책을 받게 됩니다.
...
<Steps>
  ...
  <Step title="지속적으로 개선하기">
    팀이 새로운 문제 상황을 마주할 때마다 정책을 추가하고 푸시하세요. 다음 `git pull` 시 모든 팀원이 업데이트를 받게 됩니다. 이 정책들은 팀과 함께 성장하는 살아있는 품질 기준이 됩니다.
  </Step>
</Steps>
...
저장소의 [`examples/`](https://github.com/exospherehost/failproofai/tree/main/examples) 디렉토리에는 다음이 포함되어 있습니다:

| 파일 | 내용 |
|------|------|
| `policies-basic.js` | 기본 정책 — 프로덕션 쓰기, force-push, 파이프 스크립트 차단 |
| `policies-notification.js` | 유휴 알림 및 세션 종료 시 Slack 알림 |
| `policies-advanced/index.js` | 트랜지티브 임포트, 비동기 훅, PostToolUse 출력 스크러빙, Stop 이벤트 처리 |