...
일반적인 시나리오에 바로 사용할 수 있는 예시 모음입니다. 각 예시에서는 설치 방법과 예상 동작을 설명합니다.
...
<Steps>
  ...
  <Step title="Claude Code 실행">
    ...
    이제 모든 도구 호출 시 정책이 자동으로 실행됩니다. Claude에게 `sudo rm -rf /`를 실행해 달라고 요청해 보세요 — 차단됩니다.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="에이전트에 훅 구성">
    ...
    ```bash
    failproofai --hook PreToolUse   # 각 도구 호출 전에 실행
    failproofai --hook PostToolUse  # 각 도구 호출 후에 실행
    ```
  </Step>
...
가장 일반적인 설정 — 에이전트가 되돌릴 수 없는 손상을 일으키지 못하도록 방지합니다.
...
customPolicies.add({
  ...
  fn: async (ctx) => {
    ...
    try {
      ...
      // Slack에 연결할 수 없는 경우에도 에이전트를 절대 차단하지 않음
    }
...
| 파일 | 내용 |
|------|------|
| `policies-basic.js` | 기본 정책 — 프로덕션 쓰기 차단, 강제 푸시 차단, 파이프된 스크립트 차단 |
