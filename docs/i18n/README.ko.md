...
AI 에이전트를 안정적으로 유지하고, 목표에 집중하며, 자율적으로 실행되도록 정책을 관리하는 가장 쉬운 방법 - **Claude Code**, **OpenAI Codex**, **GitHub Copilot CLI** _(베타)_, **Cursor Agent** _(베타)_, **OpenCode** _(베타)_, **Pi** _(베타)_, **Gemini CLI** _(베타)_ 및 **Agents SDK** 를 지원합니다.

<p align="center">
  <img src="failproofai-hq.gif" alt="Failproof AI 동작 모습" width="800" />
</p>
...
> 하나 또는 여러 CLI를 조합하여 훅을 설치하세요: `failproofai policies --install --cli opencode pi gemini` (또는 `--cli claude codex copilot cursor opencode pi gemini`). `--cli`를 생략하면 설치된 CLI를 자동으로 감지하여 선택을 안내합니다. **GitHub Copilot CLI, Cursor Agent, OpenCode, Pi, Gemini CLI 지원은 베타 단계로, 테스트가 진행 중입니다.**

- **39가지 기본 제공 정책** - 일반적인 에이전트 오류 패턴을 즉시 감지합니다. 파괴적인 명령 차단, 시크릿 유출 방지, 에이전트를 프로젝트 범위 내로 제한, 루프 감지 등의 기능을 제공합니다.
- **커스텀 정책** - JavaScript로 나만의 신뢰성 규칙을 작성하세요. `allow`/`deny`/`instruct` API를 활용해 규칙 준수 강제, 드리프트 방지, 작업 게이트 적용, 외부 시스템 연동 등이 가능합니다.
- **간편한 설정** - 코드 없이 모든 정책을 세밀하게 조정하세요. 프로젝트별 또는 전역으로 허용 목록, 보호 브랜치, 임계값을 설정할 수 있습니다. 3단계 범위 설정이 자동으로 병합됩니다.
- **에이전트 모니터** - 자리를 비운 사이 에이전트가 무엇을 했는지 확인하세요. 세션을 탐색하고, 모든 도구 호출을 검사하며, 정책이 발동된 지점을 정확히 검토할 수 있습니다.

모든 것이 로컬에서 실행되며 - 어떤 데이터도 외부로 전송되지 않습니다.
...
`~/.claude/settings.json`에 훅 항목을 작성합니다. 이제 Claude Code는 각 도구 호출 전후에 failproofai를 실행합니다.
...
**3가지 설정 범위**가 자동으로 병합됩니다(프로젝트 → 로컬 → 전역). 전체 병합 규칙은 [docs/configuration.mdx](docs/configuration.mdx)를 참고하세요.
...
| 정책 | 설명 | 설정 가능 |
|------|------|:---------:|
| `block-sudo` | 에이전트가 특권 시스템 명령을 실행하지 못하도록 차단 | `allowPatterns` |
...
| `sanitize-jwt` | JWT 토큰이 에이전트 컨텍스트에 노출되지 않도록 차단 | |
| `sanitize-api-keys` | API 키가 에이전트 컨텍스트에 노출되지 않도록 차단 | `additionalPatterns` |
| `sanitize-connection-strings` | 데이터베이스 자격증명이 에이전트 컨텍스트에 노출되지 않도록 차단 | |
...
| `block-push-master` | main/master에 실수로 푸시하지 못하도록 방지 | `protectedBranches` |
...
전체 정책 설명 및 파라미터 참고: [docs/built-in-policies.mdx](docs/built-in-policies.mdx)
...
| `toolName` | `string` | 호출된 도구 (`"Bash"`, `"Write"`, `"Read"`, …) |
...
커스텀 훅은 전이적 로컬 임포트, async/await, `process.env` 접근을 지원합니다. 오류 발생 시 fail-open 방식으로 처리됩니다(`~/.failproofai/hook.log`에 기록되며, 기본 제공 정책은 계속 실행됩니다). 전체 가이드는 [docs/custom-hooks.mdx](docs/custom-hooks.mdx)를 참고하세요.

### 컨벤션 기반 정책

`.failproofai/policies/` 디렉터리에 `*policies.{js,mjs,ts}` 파일을 추가하면 자동으로 로드됩니다 — 별도의 플래그나 설정 변경이 필요하지 않습니다. 해당 디렉터리를 git에 커밋하면 팀원 모두가 동일한 품질 기준을 자동으로 적용받습니다.
...
두 수준 모두 로드됩니다(합집합). 파일은 각 디렉터리 내에서 알파벳 순으로 로드됩니다. 순서를 제어하려면 `01-`, `02-` 등의 접두사를 사용하세요. 팀이 새로운 오류 패턴을 발견하면 정책을 추가하고 푸시하면 됩니다 — 팀원 모두가 다음 풀 시 업데이트를 받게 됩니다. 바로 사용 가능한 예시는 [examples/convention-policies/](examples/convention-policies/)를 참고하세요.
...
`http://localhost:3000`에서 Mintlify 문서 사이트가 열립니다. docs 디렉터리를 마운트하면 컨테이너가 변경 사항을 감시합니다:
...
다른 모든 저장소에서는 `npx -y failproofai` 사용이 권장되며, 다음 명령으로 설치할 수 있습니다:
...
**ExosphereHost: 에이전트를 위한 신뢰성 연구소**가 구축하고 유지 관리합니다. 저희는 자체 에이전트, 소프트웨어, 전문 지식을 통해 기업과 스타트업이 AI 에이전트의 신뢰성을 향상시킬 수 있도록 돕습니다. 자세한 내용은 [exosphere.host](https://exosphere.host)에서 확인하세요.
