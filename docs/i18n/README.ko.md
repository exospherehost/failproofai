...
AI 에이전트를 안정적으로 유지하고, 작업에 집중시키며, 자율적으로 실행할 수 있도록 정책을 관리하는 가장 쉬운 방법 — **Claude Code** 및 **Agents SDK** 지원.

<p align="center">
  <img src="failproofai-hq.gif" alt="Failproof AI in action" width="800" />
</p>

- **39가지 기본 제공 정책** - 흔한 에이전트 오류 패턴을 즉시 감지합니다. 파괴적인 명령 차단, 시크릿 유출 방지, 에이전트를 프로젝트 경계 안에 제한, 루프 감지 등을 제공합니다.
- **커스텀 정책** - JavaScript로 나만의 신뢰성 규칙을 작성하세요. `allow`/`deny`/`instruct` API를 활용해 컨벤션을 적용하고, 드리프트를 방지하며, 작업을 승인하거나 외부 시스템과 연동할 수 있습니다.
- **간편한 설정** - 코드 작성 없이 모든 정책을 조정하세요. 프로젝트별 또는 전역으로 허용 목록, 보호 브랜치, 임계값을 설정할 수 있습니다. 세 가지 범위의 설정이 자동으로 병합됩니다.
- **에이전트 모니터** - 자리를 비운 동안 에이전트가 무엇을 했는지 확인하세요. 세션을 탐색하고, 모든 툴 호출을 검사하며, 정책이 적용된 정확한 위치를 확인할 수 있습니다.
...
`~/.claude/settings.json`에 훅 항목을 작성합니다. 이후 Claude Code는 각 툴 호출 전후로 failproofai를 실행합니다.
...
| `block-curl-pipe-sh` | 에이전트가 신뢰할 수 없는 스크립트를 쉘로 파이프하는 것 방지 | |
...
| `sanitize-connection-strings` | 데이터베이스 자격증명이 에이전트 컨텍스트로 유출되는 것 방지 | |
...
| `block-read-outside-cwd` | 에이전트를 프로젝트 경계 안에 제한 | `allowPaths` |
...
전체 정책 세부 정보 및 파라미터 참조: [docs/built-in-policies.mdx](docs/built-in-policies.mdx)

---

## 커스텀 정책

에이전트를 안정적이고 작업에 집중하도록 유지하는 나만의 정책을 작성하세요:
...
커스텀 훅은 전이적 로컬 임포트, async/await, `process.env` 접근을 지원합니다. 오류가 발생해도 실패 시 허용(fail-open) 방식으로 처리됩니다 (`~/.failproofai/hook.log`에 기록되며 기본 제공 정책은 계속 실행됩니다). 전체 가이드는 [docs/custom-hooks.mdx](docs/custom-hooks.mdx)를 참조하세요.

### 컨벤션 기반 정책

`.failproofai/policies/` 디렉터리에 `*policies.{js,mjs,ts}` 파일을 넣으면 자동으로 로드됩니다 — 플래그나 설정 변경이 필요하지 않습니다. 해당 디렉터리를 git에 커밋하면 팀 모든 구성원이 동일한 품질 기준을 자동으로 적용받습니다.
...
두 수준 모두 로드됩니다 (합집합). 파일은 각 디렉터리 내에서 알파벳순으로 로드됩니다. 순서를 제어하려면 `01-`, `02-` 등의 접두사를 사용하세요. 팀이 새로운 오류 패턴을 발견할 때마다 정책을 추가하고 푸시하면 — 다음 풀 시 모든 팀원이 업데이트를 받게 됩니다. 바로 사용할 수 있는 예시는 [examples/convention-policies/](examples/convention-policies/)를 참조하세요.

---

## 텔레메트리

Failproof AI는 기능 사용 현황 파악을 위해 PostHog를 통해 익명 사용 텔레메트리를 수집합니다. 세션 내용, 파일명, 툴 입력, 개인 정보는 절대 전송되지 않습니다.
...
다른 모든 저장소에서는 `npx -y failproofai`를 권장하며, 다음 명령으로 설치할 수 있습니다:
...
**ExosphereHost: Reliability Research Lab for Your Agents**가 개발 및 유지 관리합니다. 저희는 자체 에이전트, 소프트웨어, 전문 지식을 통해 기업과 스타트업이 AI 에이전트의 신뢰성을 향상할 수 있도록 지원합니다. 자세한 내용은 [exosphere.host](https://exosphere.host)에서 확인하세요.
