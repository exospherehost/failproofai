...
管理策略的最简方式，让你的 AI 智能体保持可靠、专注于任务、自主运行 —— 适用于 **Claude Code** 与 **Agents SDK**。
...
| `block-curl-pipe-sh` | 阻止智能体将不可信脚本通过管道传给 shell 执行 | |
...
自定义 hook 支持传递式本地导入、async/await，以及对 `process.env` 的访问。错误为失败开放模式（记录到 `~/.failproofai/hook.log`，内置策略继续执行）。完整指南请参阅 [docs/custom-hooks.mdx](docs/custom-hooks.mdx)。
...
两个级别均会加载（取并集）。同一目录内的文件按字母顺序加载。可使用 `01-`、`02-` 等前缀控制加载顺序。当团队发现新的故障模式时，添加一条策略并推送 —— 所有人下次拉取时即可获得更新。可参考 [examples/convention-policies/](examples/convention-policies/) 中的即用示例。

---

## 遥测

Failproof AI 通过 PostHog 收集匿名使用遥测数据，以了解功能使用情况。会话内容、文件名、工具输入或任何个人信息均不会被发送。
...
由 **ExosphereHost：智能体可靠性研究实验室** 构建与维护。我们通过自有智能体、软件和专业知识，帮助企业和初创公司提升 AI 智能体的可靠性。了解更多请访问 [exosphere.host](https://exosphere.host)。
