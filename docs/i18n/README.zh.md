...
**为编程智能体提供运行时故障解决方案。**
接入 Claude Code 和 Codex，在循环调用、危险操作和密钥泄露演变为事故之前将其拦截。零延迟，本地运行。
...
> 可为一个或多个 CLI 安装 hooks：`failproofai policies --install --cli opencode pi gemini`（或 `--cli claude codex copilot cursor opencode pi gemini`）。省略 `--cli` 则自动检测已安装的 CLI 并提示选择。**GitHub Copilot CLI、Cursor Agent、OpenCode、Pi 和 Gemini CLI 支持目前处于 Beta 阶段，测试仍在持续进行。**
...
30 条内置策略即刻生效。Dashboard 地址：`localhost:8020`。
...
| `block-work-on-main` | 在 `main` / `master` 分支上提交、合并、变基 |
| `block-rm-rf` | 递归删除文件 |
| `sanitize-api-keys` | API 密钥泄露到 agent 上下文中 |

→ [查看全部 30 条内置策略](https://docs.befailproof.ai/built-in-policies)

---

## 自定义策略

将文件放入 `.failproofai/policies/` 目录即可自动加载，无需任何额外参数。提交到代码仓库后，团队成员下次拉取即可同步生效。
...
|---|---|
| `allow()` | 允许该操作 |
| `deny(message)` | 阻止该操作——消息返回给 agent |
| `instruct(message)` | 放行，但在 agent 下一次提示中附加上下文信息 |
...
agent 的每次工具调用均在本地记录日志。Dashboard 展示了执行了哪些操作、哪些被拦截、策略向 agent 传达了什么——让你在出现问题时无需猜测。→ [Dashboard 指南](https://docs.befailproof.ai/dashboard)
...
MIT 附加 [Commons Clause](https://commonsclause.com/) — 个人及内部使用免费；将 failproofai 本身作为商品转售需另行签订协议。完整条款请参阅 [LICENSE](./LICENSE)。
