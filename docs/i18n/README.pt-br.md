...
A maneira mais fácil de gerenciar políticas que mantêm seus agentes de IA confiáveis, focados na tarefa e em execução autônoma — para **Claude Code**, **OpenAI Codex** e o **Agents SDK**.
...
> Instale hooks para um ou ambos: `failproofai policies --install --cli codex` (ou `--cli claude codex`). Omita `--cli` para detectar automaticamente os CLIs instalados e exibir um prompt.

- **39 Políticas Integradas** — Detecte falhas comuns de agentes logo de início. Bloqueie comandos destrutivos, evite vazamento de segredos, mantenha agentes dentro dos limites do projeto, detecte loops e muito mais.
...
- **Configuração Simples** — Ajuste qualquer política sem escrever código. Defina listas de permissões, branches protegidos e limites por projeto ou globalmente. Três escopos de configuração são mesclados automaticamente.
...
Insere entradas de hook no `~/.claude/settings.json`. O Claude Code passará a invocar o failproofai antes e depois de cada chamada de ferramenta.
...
| `warn-git-amend` | Lembra agentes antes de fazer amend em commits | |
...
Hooks personalizados suportam importações locais transitivas, async/await e acesso a `process.env`. Erros são tratados de forma permissiva (registrados em `~/.failproofai/hook.log`, as políticas integradas continuam). Consulte [docs/custom-hooks.mdx](docs/custom-hooks.mdx) para o guia completo.

### Políticas baseadas em convenção

Coloque arquivos `*policies.{js,mjs,ts}` em `.failproofai/policies/` e eles serão carregados automaticamente — sem flags ou alterações de configuração. Faça commit do diretório no git e todos os membros da equipe terão os mesmos padrões de qualidade automaticamente.
...
Ambos os níveis são carregados (união). Os arquivos são carregados em ordem alfabética dentro de cada diretório. Use prefixos como `01-`, `02-`, etc. para controlar a ordem. Conforme sua equipe descobre novos modos de falha, adicione uma política e faça push — todos recebem a atualização no próximo pull. Veja [examples/convention-policies/](examples/convention-policies/) para exemplos prontos para uso.
