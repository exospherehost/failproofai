...
La façon la plus simple de gérer les politiques qui maintiennent vos agents IA fiables, concentrés sur leurs tâches et capables de fonctionner de manière autonome — pour **Claude Code**, **OpenAI Codex** et l'**Agents SDK**.
...
<p align="center">
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <strong>+ d'autres à venir prochainement</strong>
</p>

> Installez les hooks pour l'un ou les deux : `failproofai policies --install --cli codex` (ou `--cli claude codex`). Omettez `--cli` pour détecter automatiquement les CLI installés et afficher une invite.

- **39 politiques intégrées** - Détectez les modes d'échec courants des agents dès l'installation. Bloquez les commandes destructrices, prévenez les fuites de secrets, maintenez les agents dans les limites du projet, détectez les boucles, et bien plus encore.
- **Politiques personnalisées** - Rédigez vos propres règles de fiabilité en JavaScript. Utilisez l'API `allow`/`deny`/`instruct` pour imposer des conventions, prévenir les dérives, contrôler les opérations ou s'intégrer à des systèmes externes.
- **Configuration simple** - Ajustez n'importe quelle politique sans écrire de code. Définissez des listes d'autorisation, des branches protégées et des seuils par projet ou de façon globale. La fusion à trois niveaux de configuration est automatique.
...
- Node.js >= 20.9.0
- Bun >= 1.3.0 (optionnel — uniquement nécessaire pour le développement / la compilation depuis les sources)
...
| Portée | Commande | Fichier de destination |
|--------|---------|-----------------|
...
|--------|-------------|:---:|
| `block-sudo` | Empêche les agents d'exécuter des commandes système avec privilèges élevés | `allowPatterns` |
| `block-rm-rf` | Prévient la suppression récursive accidentelle de fichiers | `allowPaths` |
...
| `sanitize-jwt` | Empêche les tokens JWT de fuiter dans le contexte de l'agent | |
...
| `block-push-master` | Prévient les poussées accidentelles vers main/master | `protectedBranches` |
...
| `warn-large-file-write` | Détecte les écritures de fichiers inopinément volumineuses | `thresholdKb` |
| `warn-package-publish` | Détecte les `npm publish` accidentels | |
| `warn-background-process` | Détecte les lancements non intentionnels de processus en arrière-plan | |
| `warn-global-package-install` | Détecte les installations non intentionnelles de paquets globaux | |
| …et plus encore | | |
...
Les hooks personnalisés prennent en charge les imports locaux transitifs, async/await, et l'accès à `process.env`. Les erreurs sont de type fail-open (consignées dans `~/.failproofai/hook.log`, les politiques intégrées continuent). Consultez [docs/custom-hooks.mdx](docs/custom-hooks.mdx) pour le guide complet.

### Politiques basées sur des conventions

Déposez des fichiers `*policies.{js,mjs,ts}` dans `.failproofai/policies/` et ils sont chargés automatiquement — sans indicateurs ni modifications de configuration. Commitez le répertoire dans git et chaque membre de l'équipe bénéficie automatiquement des mêmes standards de qualité.
...
Les deux niveaux sont chargés (union). Les fichiers sont chargés par ordre alphabétique au sein de chaque répertoire. Préfixez avec `01-`, `02-`, etc. pour contrôler l'ordre. Au fur et à mesure que votre équipe découvre de nouveaux modes d'échec, ajoutez une politique et poussez — tout le monde reçoit la mise à jour à la prochaine synchronisation. Consultez [examples/convention-policies/](examples/convention-policies/) pour des exemples prêts à l'emploi.

---

## Télémétrie

Failproof AI collecte des données de télémétrie d'utilisation anonymes via PostHog afin de comprendre l'utilisation des fonctionnalités. Aucun contenu de session, nom de fichier, paramètre d'outil ou information personnelle n'est jamais transmis.
...
| Guide | Description |
|-------|-------------|
| [Démarrage](docs/getting-started.mdx) | Installation et premières étapes |
...
Ouvre le site de documentation Mintlify à `http://localhost:3000`. Le conteneur surveille les modifications si vous montez le répertoire docs :
...
Le fichier `.claude/settings.json` de ce dépôt utilise `bun ./bin/failproofai.mjs --hook <EventType>` au lieu de la commande standard `npx -y failproofai`. Cela est dû au fait qu'exécuter `npx -y failproofai` au sein même du projet failproofai crée un conflit d'auto-référencement.
