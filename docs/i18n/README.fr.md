...
La solution la plus simple pour gérer les politiques qui maintiennent vos agents IA fiables, concentrés sur leurs tâches et autonomes — pour **Claude Code** et le **Agents SDK**.
...
| `block-failproofai-commands` | Prévient la désinstallation automatique | |
...
| `sanitize-private-key-content` | Masque les blocs de clés privées PEM dans les sorties | |
| `sanitize-bearer-tokens` | Masque les tokens Bearer d'autorisation dans les sorties | |
...
| `block-secrets-write` | Prévient l'écriture dans les fichiers de clés privées et certificats | `additionalPatterns` |
| `block-push-master` | Prévient les push accidentels vers main/master | `protectedBranches` |
| `block-work-on-main` | Maintient les agents en dehors des branches protégées | `protectedBranches` |
| `block-force-push` | Prévient les `git push --force` | |
...
| `warn-destructive-sql` | Détecte les requêtes DROP/DELETE SQL avant exécution | |
...
| `warn-global-package-install` | Détecte les installations globales de paquets non intentionnelles | |
| …et bien d'autres | | |
...
Rédigez vos propres politiques pour maintenir vos agents fiables et concentrés sur leurs tâches :
...
| `session.transcriptPath` | `string` | Chemin vers le fichier de transcription de session |
...
Déposez des fichiers `*policies.{js,mjs,ts}` dans `.failproofai/policies/` et ils sont chargés automatiquement — aucun paramètre ni modification de configuration nécessaire. Committez le répertoire dans git et chaque membre de l'équipe bénéficie automatiquement des mêmes standards de qualité.
...
Les deux niveaux sont chargés (union). Les fichiers sont chargés par ordre alphabétique dans chaque répertoire. Préfixez avec `01-`, `02-`, etc. pour contrôler l'ordre. Au fur et à mesure que votre équipe découvre de nouveaux modes de défaillance, ajoutez une politique et poussez — tout le monde reçoit la mise à jour à leur prochain pull. Consultez [examples/convention-policies/](examples/convention-policies/) pour des exemples prêts à l'emploi.

---

## Télémétrie

Failproof AI collecte des données de télémétrie d'utilisation anonymes via PostHog pour comprendre l'utilisation des fonctionnalités. Aucun contenu de session, nom de fichier, entrée d'outil ni information personnelle n'est jamais envoyé.
...
Développé et maintenu par **ExosphereHost : Reliability Research Lab for Your Agents**. Nous aidons les entreprises et les startups à améliorer la fiabilité de leurs agents IA grâce à nos propres agents, logiciels et expertise. En savoir plus sur [exosphere.host](https://exosphere.host).
