...
La forma más sencilla de gestionar políticas que mantienen a tus agentes de IA confiables, enfocados en su tarea y operando de forma autónoma — para **Claude Code** y el **Agents SDK**.

<p align="center">
  <img src="failproofai-hq.gif" alt="Failproof AI en acción" width="800" />
</p>

- **30 políticas integradas** - Detecta los modos de fallo más comunes de los agentes desde el primer momento. Bloquea comandos destructivos, previene fugas de secretos, mantiene a los agentes dentro de los límites del proyecto, detecta bucles y mucho más.
- **Políticas personalizadas** - Escribe tus propias reglas de fiabilidad en JavaScript. Usa la API `allow`/`deny`/`instruct` para aplicar convenciones, prevenir derivas, controlar operaciones o integrarte con sistemas externos.
- **Configuración sencilla** - Ajusta cualquier política sin escribir código. Define listas de permisos, ramas protegidas y umbrales por proyecto o de forma global. La configuración de tres ámbitos se combina automáticamente.
- **Monitor de agentes** - Consulta qué hicieron tus agentes mientras no estabas. Explora sesiones, inspecciona cada llamada a herramientas y revisa exactamente dónde se activaron las políticas.
...
- Node.js >= 20.9.0
- Bun >= 1.3.0 (opcional — solo necesario para desarrollo o para compilar desde el código fuente)
...
Escribe las entradas de hooks en `~/.claude/settings.json`. Claude Code invocará failproofai antes y después de cada llamada a una herramienta.
...
Abre `http://localhost:8020` — explora sesiones, inspecciona logs y gestiona políticas.

### 3. Revisar las políticas activas
...
**Los tres ámbitos de configuración** se combinan automáticamente (proyecto → local → global). Consulta [docs/configuration.mdx](docs/configuration.mdx) para conocer las reglas completas de combinación.
...
| `sanitize-jwt` | Impide que los tokens JWT se filtren en el contexto del agente | |
| `sanitize-api-keys` | Impide que las claves de API se filtren en el contexto del agente | `additionalPatterns` |
| `sanitize-connection-strings` | Impide que las credenciales de base de datos se filtren en el contexto del agente | |
| `sanitize-private-key-content` | Oculta bloques de clave privada PEM en la salida | |
| `sanitize-bearer-tokens` | Oculta tokens Bearer de Authorization en la salida | |
| `block-env-files` | Evita que los agentes lean archivos .env | |
| `protect-env-vars` | Impide que los agentes impriman variables de entorno | |
...
| `warn-destructive-sql` | Detecta sentencias SQL DROP/DELETE antes de su ejecución | |
| `warn-schema-alteration` | Detecta ALTER TABLE antes de su ejecución | |
...
Escribe tus propias políticas para mantener a los agentes fiables y enfocados en su tarea:
...
Los hooks personalizados admiten importaciones locales transitivas, async/await y acceso a `process.env`. Los errores son de tipo fail-open (se registran en `~/.failproofai/hook.log` y las políticas integradas continúan). Consulta [docs/custom-hooks.mdx](docs/custom-hooks.mdx) para la guía completa.

### Políticas basadas en convenciones

Coloca archivos `*policies.{js,mjs,ts}` en `.failproofai/policies/` y se cargarán automáticamente — sin necesidad de flags ni cambios en la configuración. Incluye el directorio en git y todos los miembros del equipo obtendrán automáticamente los mismos estándares de calidad.
...
Ambos niveles se cargan (unión). Los archivos se cargan en orden alfabético dentro de cada directorio. Usa prefijos `01-`, `02-`, etc. para controlar el orden. A medida que tu equipo descubre nuevos modos de fallo, añade una política y haz push — todos reciben la actualización en su próximo pull. Consulta [examples/convention-policies/](examples/convention-policies/) para ver ejemplos listos para usar.

---

## Telemetría

Failproof AI recopila telemetría de uso anónima a través de PostHog para comprender el uso de las funcionalidades. Nunca se envía el contenido de las sesiones, nombres de archivos, entradas de herramientas ni información personal.
...
| [Configuración](docs/configuration.mdx) | Formato del archivo de configuración y combinación de ámbitos |
...
| [Pruebas](docs/testing.mdx) | Ejecución de pruebas y escritura de nuevas |
...
Abre el sitio de documentación de Mintlify en `http://localhost:3000`. El contenedor detecta cambios si montas el directorio de documentación:
...
## Nota para contribuidores de failproofai

El archivo `.claude/settings.json` de este repositorio usa `bun ./bin/failproofai.mjs --hook <EventType>` en lugar del comando estándar `npx -y failproofai`. Esto se debe a que ejecutar `npx -y failproofai` dentro del propio proyecto failproofai genera un conflicto de autorreferencia.
