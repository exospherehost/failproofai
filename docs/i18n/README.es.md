...
La forma más sencilla de gestionar políticas que mantienen a tus agentes de IA fiables, enfocados en sus tareas y operando de forma autónoma — para **Claude Code**, **OpenAI Codex** y el **Agents SDK**.
...
> Instala hooks para uno o ambos: `failproofai policies --install --cli codex` (o `--cli claude codex`). Omite `--cli` para detectar automáticamente los CLIs instalados y mostrar un aviso.

- **39 políticas integradas** - Detecta los modos de fallo más comunes de los agentes sin configuración adicional. Bloquea comandos destructivos, evita la filtración de secretos, mantiene a los agentes dentro de los límites del proyecto, detecta bucles y mucho más.
- **Políticas personalizadas** - Escribe tus propias reglas de fiabilidad en JavaScript. Usa la API `allow`/`deny`/`instruct` para hacer cumplir convenciones, prevenir derivas, controlar operaciones o integrarte con sistemas externos.
- **Configuración sencilla** - Ajusta cualquier política sin escribir código. Define listas de permitidos, ramas protegidas y umbrales por proyecto o de forma global. Los tres ámbitos de configuración se combinan automáticamente.
- **Monitor de agentes** - Consulta lo que hicieron tus agentes mientras no estabas. Explora sesiones, inspecciona cada llamada a herramientas y revisa exactamente dónde se activaron las políticas.
...
Escribe entradas de hook en `~/.claude/settings.json`. A partir de ahora, Claude Code invocará failproofai antes y después de cada llamada a una herramienta.
...
Abre `http://localhost:8020` — explora sesiones, inspecciona registros y gestiona políticas.

### 3. Revisar qué está activo
...
**Los tres ámbitos de configuración** se combinan automáticamente (proyecto → local → global). Consulta [docs/configuration.mdx](docs/configuration.mdx) para ver las reglas completas de combinación.
...
|----------|-------------|:---:|
| `block-sudo` | Impide que los agentes ejecuten comandos del sistema con privilegios | `allowPatterns` |
| `block-rm-rf` | Evita la eliminación recursiva accidental de archivos | `allowPaths` |
| `block-curl-pipe-sh` | Impide que los agentes redirijan scripts no confiables al shell | |
| `block-failproofai-commands` | Evita la autodesinstalación | |
...
| `sanitize-private-key-content` | Elimina bloques de clave privada PEM de la salida | |
| `sanitize-bearer-tokens` | Elimina tokens Bearer de Authorization de la salida | |
...
| `block-secrets-write` | Evita escrituras en archivos de clave privada y certificados | `additionalPatterns` |
| `block-push-master` | Evita pushes accidentales a main/master | `protectedBranches` |
| `block-work-on-main` | Mantiene a los agentes fuera de las ramas protegidas | `protectedBranches` |
| `block-force-push` | Evita `git push --force` | |
...
| `warn-destructive-sql` | Detecta sentencias DROP/DELETE en SQL antes de ejecutarlas | |
...
Escribe tus propias políticas para mantener a los agentes fiables y enfocados en sus tareas:
...
Los hooks personalizados admiten importaciones locales transitivas, async/await y acceso a `process.env`. Los errores son fail-open (se registran en `~/.failproofai/hook.log` y las políticas integradas continúan funcionando). Consulta [docs/custom-hooks.mdx](docs/custom-hooks.mdx) para la guía completa.

### Políticas basadas en convenciones

Coloca archivos `*policies.{js,mjs,ts}` en `.failproofai/policies/` y se cargarán automáticamente — sin flags ni cambios de configuración. Haz commit del directorio en git y todos los miembros del equipo obtendrán los mismos estándares de calidad de forma automática.

```text
# Nivel de proyecto — en git, compartido con el equipo
...
Ambos niveles se cargan juntos (unión). Los archivos se cargan en orden alfabético dentro de cada directorio. Usa el prefijo `01-`, `02-`, etc. para controlar el orden. A medida que el equipo descubra nuevos modos de fallo, añade una política y haz push — todos recibirán la actualización en su próximo pull. Consulta [examples/convention-policies/](examples/convention-policies/) para ver ejemplos listos para usar.
