...
La forma más sencilla de gestionar políticas que mantienen a tus agentes de IA fiables, centrados en su tarea y funcionando de forma autónoma — para **Claude Code**, **OpenAI Codex**, **GitHub Copilot CLI** _(beta)_, **Cursor Agent** _(beta)_, **OpenCode** _(beta)_, **Pi** _(beta)_, **Gemini CLI** _(beta)_ y el **Agents SDK**.
...
> Instala hooks para uno o cualquier combinación: `failproofai policies --install --cli opencode pi gemini` (o `--cli claude codex copilot cursor opencode pi gemini`). Omite `--cli` para detectar automáticamente los CLIs instalados y recibir un aviso interactivo. **La compatibilidad con GitHub Copilot CLI, Cursor Agent, OpenCode, Pi y Gemini CLI está en beta — las pruebas continúan.**

- **39 políticas integradas** - Detecta los fallos más comunes de los agentes desde el primer uso. Bloquea comandos destructivos, evita la filtración de secretos, mantiene a los agentes dentro de los límites del proyecto, detecta bucles y mucho más.
...
- **Configuración sencilla** - Ajusta cualquier política sin escribir código. Define listas de permitidos, ramas protegidas y umbrales por proyecto o de forma global. La configuración en tres niveles se fusiona automáticamente.
- **Monitor de agentes** - Consulta lo que hicieron tus agentes mientras no estabas. Navega por las sesiones, inspecciona cada llamada a herramientas y revisa exactamente dónde se activaron las políticas.
...
Escribe las entradas de hook en `~/.claude/settings.json`. Claude Code invocará failproofai antes y después de cada llamada a una herramienta.
...
**Los tres niveles de configuración** se fusionan automáticamente (proyecto → local → global). Consulta [docs/configuration.mdx](docs/configuration.mdx) para ver las reglas completas de fusión.
...
|----------|-------------|:---:|
| `block-sudo` | Impide que los agentes ejecuten comandos de sistema con privilegios elevados | `allowPatterns` |
| `block-rm-rf` | Evita la eliminación recursiva accidental de archivos | `allowPaths` |
| `block-curl-pipe-sh` | Impide que los agentes canalicen scripts no confiables hacia el shell | |
...
| `sanitize-api-keys` | Impide que las claves de API se filtren al contexto del agente | `additionalPatterns` |
| `sanitize-connection-strings` | Impide que las credenciales de base de datos se filtren al contexto del agente | |
| `sanitize-private-key-content` | Oculta los bloques de clave privada PEM de la salida | |
| `sanitize-bearer-tokens` | Oculta los tokens Bearer de Authorization de la salida | |
...
| `block-push-master` | Evita envíos accidentales a main/master | `protectedBranches` |
...
| `warn-destructive-sql` | Detecta comandos SQL DROP/DELETE antes de ejecutarlos | |
...
| `warn-global-package-install` | Detecta instalaciones no intencionadas de paquetes globales | |
| …y más | | |
...
| [Panel de control](docs/dashboard.mdx) | Monitoriza sesiones y revisa la actividad de políticas |
