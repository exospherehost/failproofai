    ...
    customPolicies.add({
      ...
      fn: async (ctx) => {
        ...
        if (path.startsWith("/") && !path.startsWith(ctx.session?.cwd ?? "")) {
          return deny("L'agent est limité au répertoire du projet");
        }
        return allow();
      },
    });
...
- `block-curl-pipe-sh` — bloque le pipe de scripts distants vers le shell
...
## Recevoir des alertes Slack quand un agent nécessite une attention

Utilisez le hook de notification pour transmettre les alertes d'inactivité à Slack.
...
customPolicies.add({
  ...
  fn: async (ctx) => {
    ...
    if (/git\s+checkout\s+(?!-b)/.test(cmd)) {
      return deny("Restez sur la branche actuelle. Créez une nouvelle branche avec -b si nécessaire.");
    }
    return allow();
  },
});
...
## Exiger les tests avant les commits
...
## Construire un standard qualité à l'échelle de l'organisation avec des politiques de convention

La configuration la plus efficace : committez `.failproofai/policies/` dans votre dépôt avec des politiques adaptées à votre projet. Tous les membres de l'équipe les reçoivent automatiquement — sans commandes d'installation ni modifications de configuration.

<Steps>
  <Step title="Créer le répertoire et ajouter les politiques">
    ```bash
    ...
    // (ou activer la politique intégrée prefer-package-manager à la place)
    customPolicies.add({
      name: "enforce-bun",
      match: { events: ["PreToolUse"] },
      fn: async (ctx) => {
  ...
  <Step title="Continuer à améliorer">
    Au fur et à mesure que votre équipe rencontre de nouveaux modes d'échec, ajoutez des politiques et poussez-les. Tout le monde reçoit la mise à jour au prochain `git pull`. Ces politiques deviennent un standard qualité vivant qui évolue avec votre équipe.
  </Step>
</Steps>
...
|---------|-------------------|
| `policies-basic.js` | Politiques de démarrage — blocage des écritures en production, des force-push et des scripts pipés |
