...
Des exemples prêts à l'emploi pour les scénarios courants. Chacun montre comment procéder à l'installation et ce à quoi vous devez vous attendre.

---

## Configurer des hooks pour Claude Code

Failproof AI s'intègre avec Claude Code via son [système de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Lorsque vous exécutez `failproofai policies --install`, il enregistre des commandes de hook dans le fichier `settings.json` de Claude Code, qui se déclenchent à chaque appel d'outil.
...
<Steps>
  ...
  <Step title="Configurer les hooks dans votre agent">
    Transmettez les commandes de hook lors de la création de votre processus agent. Les hooks se déclenchent de la même manière que dans Claude Code — via JSON stdin/stdout :
...
- `block-curl-pipe-sh` — bloque la redirection de scripts distants vers le shell
...
Commitez une configuration au niveau du projet afin que chaque développeur de votre équipe bénéficie des mêmes politiques.
...
|---------|-----------------|
| `policies-basic.js` | Politiques de démarrage — bloquer les écritures en production, les force-push, les scripts redirigés |
