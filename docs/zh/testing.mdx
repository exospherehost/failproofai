...
failproofai 包含两套测试体系：**单元测试**（快速、使用模拟）和**端到端测试**（真实子进程调用）。
...
端到端测试将真实的 `failproofai` 二进制文件作为子进程调用，通过 stdin 传入 JSON 载荷，并对 stdout 输出和退出码进行断言。这测试了 Claude Code 所使用的完整集成链路。
...
每当修改公共 hook API（`src/hooks/custom-hooks-registry.ts`、`src/hooks/policy-helpers.ts` 或 `src/hooks/policy-types.ts`）后，需重新构建 `dist/`。
...
__tests__/e2e/
  helpers/
    ...
    fixture-env.ts      # 为每个测试创建独立的临时目录及配置文件
    payloads.ts         # 针对每种事件类型的、符合 Claude 规范的载荷工厂函数
  hooks/
...
// env.home   - 独立的 home 目录；防止真实的 ~/.failproofai 数据污染测试
...
describe("block-rm-rf (E2E)", () => {
  ...
  it("allows non-recursive rm", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe("");  // allow → stdout 为空
  });
});
```
...
- `testTimeout: 20_000` - 每个测试超时 20 秒（含二进制文件启动 + hook 评估）

使用 `forks` 池非常重要：基于线程的 worker 共享 `globalThis`，可能与生成子进程的测试相互干扰，而基于进程的 forks 可避免这一问题。

---

## 持续集成

合并前必须通过完整的 CI 流程（`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`）。端到端测试套件作为独立的 CI 任务并行运行。

完整的合并前检查清单，请参阅 [Contributing](../CONTRIBUTING.md)。