...
Claude Code 随后会在每次工具调用前，将 `failproofai --hook PreToolUse` 作为子进程调用，并通过 stdin 传入 JSON 负载。
...
对于 `PostToolUse` 事件，负载中还包含 `tool_result` 字段，其中包含工具的输出内容。

处理器强制限制 stdin 最大为 1 MB。超过此限制的负载将被丢弃，所有策略隐式允许通过。
...
`allow(message)` 允许策略在操作被允许时向 Claude 发送信息性上下文。钩子处理器将以下 JSON 写入 **stdout**（这不是配置文件——这是处理器对 Claude Code 的响应，与 deny 和 instruct 响应相同）：
...
```
- 退出码：`0`（操作被允许）
- 当多个策略返回带消息的 `allow` 时，其消息将以换行符连接为单个 `additionalContext` 字符串
...
对于典型负载，整个过程在 100ms 以内完成，无需调用 LLM。

---

## 配置加载

`src/hooks/hooks-config.ts` 实现了三级作用域配置加载。
...
合并逻辑：
- `enabledPolicies` - 三个文件去重后的并集
- `policyParams` - 按策略名为键，第一个定义该键的文件完全优先
- `customPoliciesPath` - 第一个定义它的文件优先
- `llm` - 第一个定义它的文件优先

Web 仪表板使用 `readHooksConfig()`（仅全局级）进行读写，因为它不会以项目 cwd 被调用。
...
所有策略运行完毕后：
- 若有任何 `deny` 返回，发出 deny 响应。
- 若收集到任何 `instruct` 返回，发出单个 instruct 响应，所有消息以换行符合并。
...
`src/hooks/builtin-policies.ts` 以 `BuiltinPolicyDefinition` 对象的形式定义了全部 26 条内置策略：
...
接受 `params` 的策略会声明一个 `PolicyParamsSchema`，其中包含每个参数的类型和默认值。策略评估器在调用 `fn` 之前会将解析后的值注入 `ctx.params`。策略函数在读取 `ctx.params` 时无需做空值保护，因为默认值始终会先被应用。

策略内部的模式匹配使用解析后的命令 token（argv），而非原始字符串匹配。这可以防止通过 shell 操作符注入绕过规则（例如，针对 `sudo systemctl status *` 的模式，无法通过在命令后追加 `; rm -rf /` 来绕过）。
...
如发生任何错误（文件未找到、语法错误、导入失败），错误信息会记录到 `~/.failproofai/hook.log`，加载器返回空数组。内置策略不受影响。

自定义策略在所有内置策略之后评估。自定义策略的 `deny` 仍会短路后续自定义策略（但此时所有内置策略均已执行完毕）。
...
每条记录对应一条作出非 allow 决策的策略。Allow 决策不记录日志（以保持文件精简）。
...
- 读取页面使用 React Server Components，首屏加载更快，数据获取无需客户端 bundle。
...
│   ├── log-entries.ts            # 解析 Claude 会话记录 JSONL 格式
│   ├── paths.ts                  # 解析系统路径
│   └── ...
