...
Schreibt Hook-Einträge in die Einstellungsdatei des installierten Agenten-CLI (Claude Code, OpenAI Codex oder GitHub Copilot CLI _(Beta)_), sodass failproofai Tool-Aufrufe abfängt.
...
| Flag | Beschreibung |
|------|--------------|
| `--cli claude\|codex\|copilot` | Agenten-CLI(s), für die installiert werden soll; durch Leerzeichen getrennt (z. B. `--cli claude codex copilot`) oder wiederholt angegeben. Wird weggelassen, werden installierte CLIs automatisch erkannt und eine Auswahl angezeigt. |
| `--scope user` | Installation in die benutzerweite Einstellungsdatei (Claude: `~/.claude/settings.json`; Codex: `~/.codex/hooks.json`; Copilot: `~/.copilot/hooks/failproofai.json`). Standard. |
| `--scope project` | Installation in die projektweite Einstellungsdatei (Claude: `<cwd>/.claude/settings.json`; Codex: `<cwd>/.codex/hooks.json`; Copilot: `<cwd>/.github/hooks/failproofai.json`). |
...
```bash
# Alle Standardrichtlinien global installieren (interaktiv)
