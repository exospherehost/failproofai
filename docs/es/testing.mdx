...
failproofai cuenta con dos suites de pruebas: **pruebas unitarias** (rápidas, con mocks) y **pruebas de extremo a extremo** (invocaciones reales de subprocesos).
...
# Ejecutar pruebas unitarias en modo observación
bun run test
...
Las pruebas unitarias se encuentran en `__tests__/` y usan [Vitest](https://vitest.dev) con `happy-dom`.

```text
__tests__/
  hooks/
    builtin-policies.test.ts      # Lógica de políticas para cada builtin
    hooks-config.test.ts          # Carga de configuración y fusión de ámbitos
...
Las pruebas E2E invocan el binario real de `failproofai` como un subproceso, le envían un payload JSON por stdin y verifican la salida por stdout junto con el código de salida. Esto prueba el flujo de integración completo que utiliza Claude Code.
...
Vuelve a compilar `dist/` cada vez que modifiques la API pública de hooks (`src/hooks/custom-hooks-registry.ts`, `src/hooks/policy-helpers.ts` o `src/hooks/policy-types.ts`).
...
__tests__/e2e/
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Fusión de configuración entre ámbitos project/local/global
...
// env.home   - directorio home aislado; sin filtraciones de ~/.failproofai real
...
- `environment: "node"` - no se necesitan variables globales del navegador
...
La ejecución completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) debe completarse sin errores antes de fusionar cualquier cambio. La suite E2E se ejecuta como un trabajo de CI separado en paralelo.

Consulta [Contributing](../CONTRIBUTING.md) para ver la lista de verificación completa antes de fusionar.