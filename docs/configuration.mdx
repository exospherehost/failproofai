...
Pass `--cli claude|codex|copilot|cursor|pi` to target a specific agent (space-separated or repeated for any subset):
...
When `--cli` is omitted, `failproofai` detects which agent CLIs are installed (`which claude` / `which codex` / `which copilot` / `which cursor-agent` / `which pi`):
