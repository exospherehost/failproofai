...
Pass `--cli claude-code|codex` to target a specific agent (repeatable or comma-separated for both):
...
When `--cli` is omitted, `failproofai` detects which agent CLIs are installed (`which claude` / `which codex`). If only one is present it auto-selects; if both are present you'll be prompted.
