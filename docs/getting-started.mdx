...
<Steps>
  <Step title="Enable policies">
    ...
    You can also install hooks for other CLIs (Cursor, Gemini, Copilot, etc.) using the `--cli` flag, which supports multiple values:

    ```bash
    failproofai policies --install --cli cursor gemini
    ```
  </Step>
  <Step title="Verify">
    ```bash
