...
The easiest way to manage policies that keep your AI agents reliable, on-task, and running autonomously - for **Claude Code**, **OpenAI Codex**, **GitHub Copilot CLI** _(beta)_, **Cursor Agent** _(beta)_, **OpenCode** _(beta)_ & the **Agents SDK**.
...
> Install hooks for one or any combination: `failproofai policies --install --cli opencode` (or `--cli claude codex copilot cursor opencode`). Omit `--cli` to auto-detect installed CLIs and prompt. **GitHub Copilot CLI, Cursor Agent, and OpenCode support are in beta — testing is ongoing.**
