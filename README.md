...
The easiest way to manage policies that keep your AI agents reliable, on-task, and running autonomously - for **Claude Code**, **OpenAI Codex**, **GitHub Copilot CLI** _(beta)_, **Cursor Agent** _(beta)_, **Pi** _(beta)_ & the **Agents SDK**.
...
> Install hooks for one or any combination: `failproofai policies --install --cli pi` (or `--cli claude codex copilot cursor pi`). Omit `--cli` to auto-detect installed CLIs and prompt. **GitHub Copilot CLI, Cursor Agent, and Pi support are in beta — testing is ongoing.**
