...
export type ProjectCli = "claude" | "codex" | "copilot" | "cursor" | "pi";
...
export async function getProjectFolders(): Promise<ProjectFolder[]> {
  ...
  const [claude, codex, copilot, cursor, pi] = await Promise.all([
  ...
  return mergeProjectFolders(claude, codex, copilot, cursor, pi);
}
