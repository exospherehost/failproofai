...
if (hookIdx >= 0) {
  if (!args[hookIdx + 1]) {
    ...
    console.error("Usage: failproofai --hook <event> [--cli <claude|codex|copilot|cursor|pi>]");
    process.exit(1);
  }
...
EXAMPLES
  ...
  failproofai policies --install --cli claude codex copilot cursor pi
...
`.trimStart());
    ...
    if (isInstall) {
      ...
      const VALID_CLIS = new Set(["claude", "codex", "copilot", "cursor", "pi"]);
      const cliFlagValues = [];
      ...
      for (const idx of cliFlagIdxs) {
        ...
        if (consumed === 0) {
          throw new CliError("Missing value(s) for --cli. Usage: --cli claude codex copilot cursor pi (or any subset)");
        }
      }
