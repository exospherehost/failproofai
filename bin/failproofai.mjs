...
if (hookIdx >= 0) {
  if (!args[hookIdx + 1]) {
    ...
    console.error("Usage: failproofai --hook <event> [--cli <claude|codex|copilot|cursor|opencode>]");
    process.exit(1);
  }
  ...
  const cli =
    cliArg && (cliArg === "claude" || cliArg === "codex" || cliArg === "copilot" || cliArg === "cursor" || cliArg === "opencode")
      ? cliArg
      : "claude";
  try {
...
COMMANDS
  ...
  policies --install, -i         Enable policies in agent CLI settings
    ...
    --cli claude|codex|copilot|cursor|opencode
                                   Agent CLI(s) to install for; space-separated
                                   (e.g. --cli claude codex copilot cursor opencode) or repeated.
                                   Default: detect installed CLIs and prompt.
    --scope user|project|local     Config scope to write to (default: user)
                                   (Codex / Copilot / Cursor / OpenCode support user|project only)
...
EXAMPLES
  ...
  failproofai policies --install --cli claude codex copilot cursor opencode
...
`.trimStart());
    ...
    if (isInstall) {
      ...
      const VALID_CLIS = new Set(["claude", "codex", "copilot", "cursor", "opencode"]);
      const cliFlagValues = [];
      ...
      for (const idx of cliFlagIdxs) {
        ...
        if (consumed === 0) {
          throw new CliError("Missing value(s) for --cli. Usage: --cli claude codex copilot cursor opencode (or any subset)");
        }
      }
